...
for i = 0:H/8-1
    for j = 0:W/8-1
        ...
        if inmat ==1
            if (k-1)*64+64>len
                this_message = message((k-1)*64+1:end);
                changeinmat = 1; 
            else
                this_message = message((k-1)*64+1:(k-1)*64+64);
            end
            
            nquanti_mat = in_mat2(quanti_mat,this_message);
        else
...
[out_mat,out_message] = ex_jpeg_decode1(final_Cd,final_Ac,DCTAB,ACTAB,QTAB,H,W);
imshow(out_mat);

ratio = my_Ratio(hall_gray,final_Ac,final_Cd);
disp("Compress_ratio_1 = "+ratio);
disp("PSNR_1 = "+my_PSNR(hall_gray,out_mat));
